...
namespace ComputerScienceInternalAssessment
{
    public partial class SwimmerListForm : Form
    {
        ...
        private void SearchSwimmerNameBtn_Click(object sender, EventArgs e)
        {
            ...
            if (FirstNameRadioBtn.Checked = true && word != null)
            {
                resetGrid();
                ...
                foreach (DataGridViewRow row in SwimmerListDataGridView.Rows)
                {
                    ...
                    {
                        if ((cell.ColumnIndex == 0) && (cell.Value != word)) //Set your Column Index
                        {
                            row.Visible = false;
                        }
                    }
            ...
            else if (LastNameRadioBtn.Checked = true && word != null)
            {
                resetGrid();
                ...
                foreach (DataGridViewRow row in SwimmerListDataGridView.Rows)
                {
                    foreach (DataGridViewCell cell in row.Cells)
                    {
                        if ((cell.ColumnIndex == 1) && (cell.Value != word)) //Set your Column Index
                        {
                            row.Visible = false;
                        }
                    }
            ...
            else if (FirstAndLastNameBtn.Checked = true && word != null)
            {
                ...
                foreach (DataGridViewRow row in SwimmerListDataGridView.Rows)
                {
                    //getting index of row
                    int i = row.Index;
                    valuesLastName[row.Index] = SwimmerListDataGridView.Rows[i].Cells[0].Value.ToString();
                }

                int locationFirstName = binarySearch(valuesFirstName, split[0]);
                int locationLastName = binarySearch(valuesLastName, split[1]);

                if ((locationFirstName == -1) || (locationLastName == -1) || (locationFirstName != locationLastName))
                {
                    MessageBox.Show("Swimmer with name " + word + " not found.");
                }
                else
                {
                    foreach (DataGridViewRow row in SwimmerListDataGridView.Rows)
